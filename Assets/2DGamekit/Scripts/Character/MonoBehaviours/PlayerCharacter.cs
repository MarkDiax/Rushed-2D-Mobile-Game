	...
	public class PlayerCharacter : MonoBehaviour
	{
		...
		public bool CheckForGrounded() {
			...
			if (grounded) {
				...
				if (!wasGrounded && m_MoveVector.y < -1.0f) {//only play the landing sound if falling "fast" enough (avoid small bump playing the landing sound)
		...
		public void CheckForPushing() {
			...
			if (previousPushable != null && m_CurrentPushable != previousPushable) {//we changed pushable (or don't have one anymore), stop the old one sound
		...
		public void Respawn(bool resetHealth, bool useCheckpoint) {
			...
			if (m_FlickerCoroutine != null) {//we stop flcikering for the same reason
				StopFlickering();
			}
